...
def connect_splinter_proxy(count, url, proxyIP, proxyPort, wait_on_page_time):
    ...
    #https://splinter.readthedocs.io/en/latest/http-proxies.html
    #using unauthenticated HTTP proxy with Firefox from our list of proxy_pool
    ...
    #manual configuration for Google Chrome WebDriver
    #executable_path = {'executable_path':'C:\Users\win4n\Desktop\chromedriver.exe'}
    #b = splinter.Browser('chrome',**executable_path)
    #google chrome extension -- not configured 

    #FireFox splinter WebDriver settings
    b = splinter.Browser(profile_preferences=proxy_settings)

    b.driver.set_window_size(0, 0)
    b.visit(url)

    #sleep function sets timed interval before execution of next step
    time.sleep(5)
    if len(b.html)<20000:
        b.visit(url)
    time.sleep(5)
    if len(b.html)<20000:
        b.visit(url)
    time.sleep(wait_time)     ## 5-20 minutes
    b.quit()
...
#number of attackers
max_num_windows = 5
#total run time in seconds - continues for 
total_run_time =  60              #24*3600  -> 24 hours
